...
# Cody Geary, cody@dna.caltech.edu May 2016
#
#	nt_diff.pl
#
#   -- Looks at Residue 1, Chain A and Residue 2, Chain A and reports the translation and angles between the two ref frames
#
#	-- Translates the PDB file so that Resi 1 is centered on the Z-X Ref Frame
#	-- Next, translates Resi 2 to the Z-X ref frame, and reports the translation, theta and phi angles of rotation to do so
#
#	-- The program recursively does this for all the next nt pairs, 2-3, 3-4 etc. 
#
...
print "> File Generated by Nt_diff.pl - written by Cody Geary \n";
...
print "> "; print $residue; print " Residues in the PDB file \n";

#$c = $ATOM[0]->{i};   #start at the first residue in the file
$c = 1;  #start at the residue numbered 1

$cur_chain = $ATOM[0]->{h};
print "> starting on chain: "; print $cur_chain; print "\n";
print "Residue Num , Tr_x, Tr_y, Tr_z, Rot_Z1, Rot_y, Rot_Z2 \n";
 
my $new_resi = 0;  #keep track of each residue, incase some nts are skipped in the numbering
for ($i=0;$i<$residue;$i++) {  
	foreach $ATOM ( @ATOM ) {
	...
	if ($new_resi == 1 ) {
		...
		my $position = $i+1;
		print "$position , $c_one_x , $c_one_y , $c_one_z , $angles[0] , $angles[1] , $angles[2] \n";
	
		
					
		foreach $ATOM ( @ATOM ) {			#then, for each residue we move the whole PDB of all elements >n and store them back in the array
											#we output the translation and angular change between each residue as we go through the PDB.

			if ( $ATOM->{i} >= $c ) {   #$c is the current resi, atom(i) is the resi number element in the PDB file.
			
				my $coord_x = $ATOM->{x};			my $coord_y = $ATOM->{y};			my $coord_z = $ATOM->{z};
				my @point = ();
				...
				my $roundedx =0; my $roundedy=0; my $roundedz=0;   #this section deals with the weird special PDB coord output sytle

				$roundedx = sprintf("%4.3f", $moved_coords[0]);
				$roundedy = sprintf("%4.3f", $moved_coords[1]);
				$roundedz = sprintf("%4.3f", $moved_coords[2]);
					
				$ATOM->{x} = sprintf("%8s",$roundedx);
				$ATOM->{y} = sprintf("%8s",$roundedy);
				$ATOM->{z} = sprintf("%8s",$roundedz);
			}
		}
	} else { $residue +=1;} #if we have to skip, then we have to add 1 to the numbering
	$new_resi =0;
	$c++; #increment the residue counter
}
